---
title: "fallingo 개발일지 - 2025-10-17 ~ 2025-10-22 (20개 커밋)"
date: "2025-10-22"
category: "dev-log"
description: "소셜 로그인, AI 콘텐츠 검수, GCP 프로젝트 마이그레이션"
tags: ["fallingo", "개발일지"]
author: "Su"
lang: "ko"
---

# fallingo 개발일지 - 2025-10-17 ~ 2025-10-22 (20개 커밋)

## 📝 이번 기간 작업 내용

### 인프라 및 문서 정리 (2개 커밋)
- **logo** (10/20): 로고 추가<br />
- **docs 폴더 구조 정리 및 통합** (10/17): 문서 정리

### 인증 시스템 개선 (6개 커밋)
- **GCP Identity Platform 마이그레이션 및 프론트엔드 API 통합** (10/20): Identity Platform 전환<br />
- **API 경로에서 /v1 제거** (10/20): API 경로 단순화<br />
- **회원가입 시 국가 국기를 기본 프로필 이미지로 설정** (10/21): 프로필 기본 이미지<br />
- **소셜 로그인 패키지 추가 및 린트 에러 수정** (10/21): 소셜 로그인 준비<br />
- **소셜 로그인 완전 구현 (Google, Apple, Facebook, Twitter)** (10/21): 4개 소셜 로그인<br />
- **법률 문서 다국어 API 및 Android 인증 설정 완료** (10/21): 법률 API

### AI 및 댓글 기능 (5개 커밋)
- **댓글 좋아요 기능 추가 + 불필요한 화면 삭제** (10/20): 댓글 좋아요 백엔드<br />
- **댓글 좋아요 UI 구현 완료** (10/20): 댓글 좋아요 프론트<br />
- **Gemini API 기반 AI 콘텐츠 검수 구현** (10/21): AI 검수 시스템<br />
- **AI 검수 비용 절감 시스템 구현 (85-90% 절감)** (10/21): 비용 최적화<br />
- **실시간 카메라 오버레이에 최근 등록 시간 추가** (10/21): 카메라 UI 개선

### 프론트엔드 API 연동 (3개 커밋)
- **피드 화면 실제 API 연동 구현** (10/21): 피드 API 연동<br />
- **FeedCard 파라미터 린트 에러 수정** (10/21): 린트 수정<br />
- **다국어 파일 에러 수정** (10/21): i18n 수정

### GCP 프로젝트 변경 (4개 커밋)
- **로고 이미지 확장자 변경 (jpg → png)** (10/21): 로고 수정<br />
- **GCP 프로젝트 ID 업데이트 (fallingo-449913 → fallingo-473300)** (10/21): 프로젝트 ID 변경<br />
- **GCP 프로젝트 정보 업데이트 및 데이터베이스 마이그레이션 설정** (10/22): DB 마이그레이션<br />
- **GitHub Actions에서 Poetry 사용하도록 수정** (10/22): Poetry 전환

## 💡 작업 하이라이트

**소셜 로그인 완전 구현**

10월 20일, **GCP Identity Platform**으로 마이그레이션하며 인증 시스템을 대폭 개선했습니다. API 경로에서 /v1을 제거하여 단순화했고, 회원가입 시 **국가 국기를 기본 프로필 이미지**로 설정하는 세련된 UX를 구현했습니다.

10월 21일, **Google, Apple, Facebook, Twitter 4개 소셜 로그인**을 완전히 구현했습니다. 법률 문서 다국어 API와 Android 인증 설정까지 완료하며 프로덕션 준비를 마쳤습니다.

**AI 콘텐츠 검수 비용 85-90% 절감**

**Gemini API 기반 AI 콘텐츠 검수 시스템**을 구축했습니다. 핵심은 **비용 절감 시스템**으로, AI 검수 비용을 **85-90% 절감**했습니다. 이는 스타트업에게 매우 중요한 성과입니다.

**GCP 프로젝트 마이그레이션**

10월 21일~22일, GCP 프로젝트를 **fallingo-449913 → fallingo-473300**으로 변경했습니다. 데이터베이스 마이그레이션 설정을 완료하고, GitHub Actions를 Poetry로 전환했습니다.

## 📊 개발 현황

- **인증**: Identity Platform, 4개 소셜 로그인 완성<br />
- **AI**: Gemini 검수 시스템, 85-90% 비용 절감<br />
- **프론트엔드**: 피드 API 연동, 댓글 좋아요 UI<br />
- **인프라**: GCP 프로젝트 마이그레이션, Poetry 전환

